...
oneVarCoef(varsList, dfFilt)

step_model  = stepAIC(model1, direction = "both" , trace = T, steps = 10000)
summary(step_model)

# best linear model ConsTest ~ ALLELE.FREQUENCY + STRAND.BIAS + DEPTH + QUAL + Var_Al_RelPos + Ref_Al_RelPos + meandepth + meanbaseq + meanmapq

vif_model <- glm(ConsTest ~ ALLELE.FREQUENCY + STRAND.BIAS + DEPTH + QUAL + Var_Al_RelPos + Ref_Al_RelPos + meandepth + meanbaseq + meanmapq, data = dfFilt, family = "binomial")
print(vif(vif_model))

vif_model <- glm(ConsTest ~ ALLELE.FREQUENCY + STRAND.BIAS + DEPTH + QUAL + Var_Al_RelPos + Ref_Al_RelPos + meanbaseq + meanmapq, data = dfFilt, family = "binomial")
print(vif(vif_model))
...
aucModel <- glm(ConsTest ~ ALLELE.FREQUENCY + STRAND.BIAS + DEPTH + QUAL + Var_Al_RelPos + Ref_Al_RelPos + meanbaseq + meanmapq, data = dfFilt, family = "binomial")
probs <- predict(aucModel, newdata = test_data, type = "response")
roc_obj <- roc(test_data$ConsTest ~ probs, plot = TRUE, print.auc = TRUE)
print(roc_obj$auc)

aucModel <- randomForest(formula = ConsTest ~  ALLELE.FREQUENCY + STRAND.BIAS  + QUAL + Var_Al_RelPos + Ref_Al_RelPos + meanbaseq  + meandepth, data = train_data, ntree = 500)
probs <- predict(aucModel, newdata = test_data, type = "response")
roc_obj <- roc(test_data$ConsTest ~ probs, plot = TRUE, print.auc = TRUE)
print(roc_obj$auc)
