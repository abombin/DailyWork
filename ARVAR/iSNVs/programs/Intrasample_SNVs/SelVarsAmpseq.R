...
vif_model <- glm(ConsTest ~ALLELE.FREQUENCY + STRAND.BIAS + QUAL+ DEPTH  + Var_Al_RelPos + coverage + meandepth + meanbaseq + meanmapq, data = dfFilt, family = "binomial")
print(vif(vif_model))
summary(vif_model)
...
aucModel <- glm(ConsTest ~ALLELE.FREQUENCY + DEPTH  + Var_Al_RelPos + Ref_Al_RelPos + coverage + meandepth + meanbaseq +  meanmapq  + STRAND.BIAS + QUAL, data = dfFilt, family = "binomial")
probs <- predict(aucModel, newdata = test_data, type = "response")
roc_obj <- roc(test_data$ConsTest ~ probs, plot = TRUE, print.auc = TRUE)
print(roc_obj$auc)

aucModel <- randomForest(formula = ConsTest ~ ALLELE.FREQUENCY + DEPTH  + Var_Al_RelPos + Ref_Al_RelPos + coverage + meandepth + meanbaseq +  meanmapq  + STRAND.BIAS + QUAL, data = train_data, ntree = 500)
probs <- predict(aucModel, newdata = test_data, type = "response")
roc_obj <- roc(test_data$ConsTest ~ probs, plot = TRUE, print.auc = TRUE)
print(roc_obj$auc)
