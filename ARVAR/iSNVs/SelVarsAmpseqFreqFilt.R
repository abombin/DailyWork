...
df = read.csv("IntraSnv_results/ampseq_ConsTest_freq_1_0.01.csv")
...
summary(step_model)
# best linear model ConsTest ~ ALLELE.FREQUENCY + STRAND.BIAS + QUAL +Var_Al_RelPos + Ref_Al_RelPos + coverage + meandepth + meanbaseq
...
aucModel <- glm(ConsTest ~ALLELE.FREQUENCY + STRAND.BIAS + QUAL +Var_Al_RelPos + Ref_Al_RelPos  + meandepth + meanbaseq + coverage, data = dfFilt, family = "binomial")
probs <- predict(aucModel, newdata = test_data, type = "response")
roc_obj <- roc(test_data$ConsTest ~ probs, plot = TRUE, print.auc = TRUE)
print(roc_obj$auc)

aucModel <- randomForest(formula = ConsTest ~ ALLELE.FREQUENCY + STRAND.BIAS + QUAL +Var_Al_RelPos + Ref_Al_RelPos  + meandepth + meanbaseq+ coverage, data = train_data, ntree = 500)
probs <- predict(aucModel, newdata = test_data, type = "response")
roc_obj <- roc(test_data$ConsTest ~ probs, plot = TRUE, print.auc = TRUE)
print(roc_obj$auc)
...
finalModel = glm(ConsTest ~ ALLELE.FREQUENCY + STRAND.BIAS + QUAL +Var_Al_RelPos + Ref_Al_RelPos  + meandepth + meanbaseq, data = dfFilt, family = "binomial") # can be used as final model for no frequency and frequency filtering
summary(finalModel )
